...
// Slug 校验：与后端 blog.ValidSlug 对齐——小写字母、数字、连字符与中文，不以连字符开头/结尾。
function validSlug(s: string): boolean {
  return /^[a-z0-9一-龥]+(?:-[a-z0-9一-龥]+)*$/.test(s);
}
...
describe("validSlug", () => {
  ...
  it("接受中文 slug", () => {
    expect(validSlug("欢迎来到fluxblog")).toBe(true);
    expect(validSlug("markdown-能力演示")).toBe(true);
  });
});
